...
For this function, we provide the RM-ANOVA object (```model```), the group variable we want to estimate (```list(Cleb_Congruence = "pairwise")```) as well as specifying that we are requesting the pairwise comparisons. . We are specifying the degrees of freedom so the function uses the $t-test$ instead of the $z-test$, we get these from the handy function ```insight::get_df()``` which requires the RN-ANOVA object. Additionally, we are requesting the ```fdr``` false discovery rate $p$-value correction in th `hypotheses` function with the `multcomp` argument, asking for the tests and CI to be presented for the 95% confidence level
